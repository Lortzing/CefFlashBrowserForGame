...
- **SpeedGear:** Browser speed changes use `SpeedGearController` to write a sequence-locked shared memory factor. `CefFlashBrowser.Subprocess.exe` loads `CefFlashBrowser.SpeedGear.dll`, whose native IAT hooks scale process timing APIs and repatch after `LoadLibraryW` / `LoadLibraryExW`. Both files must remain physical output files and be excluded from Costura embedding. Set `CEF_FLASH_BROWSER_SPEEDGEAR_DEBUG=1` to emit native SpeedGear debug status through `OutputDebugStringW`. Do not reintroduce the old JavaScript timer hook for speed control. If stronger coverage is needed, add a MinHook/Detours-style inline hook layer while keeping IAT patching as a supplement.
