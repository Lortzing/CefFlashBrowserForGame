...
- **Input Memory:** The "键鼠记忆"/input macro feature uses a CEF host-input backend, not JavaScript-dispatched DOM events. `ChromiumFlashBrowser` records browser child-window Win32 mouse/keyboard messages and replays through `IBrowserHost.SendMouse*Event` / `SendKeyEvent`, so Flash/PPAPI receives browser-level input. Its native keydown bridge lets `BrowserWindow` handle configured record/replay/stop shortcuts even when the CEF child HWND has focus; matched shortcut keydowns are skipped by the recorder and consumed by the WinForms message filter. Saved macros are JSON files under the user data `InputMacros` directory via `InputMacroService`; the browser toolbar opens a script manager panel that reloads that default folder, allows renaming and order changes, shows script duration/event count, and stores shortcut/replay settings in `Settings`.
- **Feature diagnostics:** SpeedGear and Input Memory diagnostics flow through `CefFlashBrowser.FlashBrowser.FeatureDiagnostics` into the app log. The experimental subprocess also writes `Logs\subprocess_yyyyMMdd.log`. Keep feature failures observable in files, not only `Debug.WriteLine`.
