...
- **Input Memory:** The "键鼠记忆" feature uses a CEF host-input backend, not JavaScript-dispatched DOM events. `ChromiumFlashBrowser` records browser child-window Win32 mouse/keyboard messages and replays through `IBrowserHost.SendMouse*Event` / `SendKeyEvent`, so Flash/PPAPI receives browser-level input. Saved macros remain JSON files under the user data `InputMacros` directory via `InputMacroService`; keep the existing event field names for compatibility with older saved scripts.
